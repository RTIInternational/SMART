...
def test_get_label_history(
    ...
    response = admin_client.get(f"/api/get_label_history/{project.pk}/")
    ...
    response = client.post(f"/api/skip_data/{datum.pk}/", request_info)
    assert "error" not in response.json() and "detail" not in response.json()

    response = client.get(f"/api/get_label_history/{project.pk}/")
    assert response.json()["data"] == []

    # have one user label something. Call label history on two users.
    request_info = {"labelID": test_labels[0].pk, "labeling_time": 3}
    datum = data[1]
    response = client.post(f"/api/annotate_data/{datum.pk}/", request_info)
    assert "error" not in response.json() and "detail" not in response.json()

    response_client = client.get(f"/api/get_label_history/{project.pk}/")
    assert response_client.json()["data"] != []

    response_admin = admin_client.get(f"/api/get_label_history/{project.pk}/")
...
def test_annotate_data(
    ...
    response = client.post(f"/api/annotate_data/{data[0].pk}/", request_info)
    assert (
...
def test_skip_data(
    ...
    assert DataLabel.objects.filter(data=data[0], was_skipped=False).count() == 0
    assert DataLabel.objects.filter(data=data[0], was_skipped=True).count() == 1
...
def test_skip_explicit_data(
    ...
    for datum in data:
        ...
        assert (
            DataLabel.objects.filter(data=datum_upated, was_skipped=False).count() == 0
        )
        assert (
            DataLabel.objects.filter(data=datum_upated, was_skipped=True).count() == 1
        )
...
def test_modify_label(
    ...
    response = client.post(f"/api/annotate_data/{data.pk}/", request_info)
    ...
    response = client.post(f"/api/modify_label/{data.pk}/", change_info)
    assert "error" not in response.json() and "detail" not in response.json()
...
def test_modify_label_to_skip(
    ...
    response = client.post(f"/api/modify_label_to_skip/{data.pk}/", change_info)
    assert "error" not in response.json() and "detail" not in response.json()

    assert DataLabel.objects.filter(data=data, was_skipped=False).count() == 0
...
def test_skew_label(
    ...
    assert data is not None
    response = client.post(f"/api/label_skew_label/{data.pk}/", request_info)
    assert (
    ...
    response = admin_client.post(f"/api/label_skew_label/{data.pk}/", request_info)
    assert "error" not in response.json() and "detail" not in response.json()
    assert DataLabel.objects.filter(data=data).count() == 1
...
def test_admin_label(
    ...
    data = get_assignments(client_profile, project, 1)[0]
    response = client.post(
        f"/api/skip_data/{data.pk}/", {"labelID": test_labels[0].pk, "labeling_time": 0}
    )
    assert "error" not in response.json() and "detail" not in response.json()

    payload = {"labelID": test_labels[0].pk}
    response = client.post(f"/api/label_admin_label/{data.pk}/", payload)
    ...
    response = admin_client.post(f"/api/label_admin_label/{data.pk}/", payload)
    assert "error" not in response.json() and "detail" not in response.json()
...
def test_label_distribution_inverted(
    ...
    response = admin_client.get(
        f"/api/label_distribution_inverted/{project.pk}/"
    ).json()
...
def test_unlabeled_table(
    ...
    response = admin_client.get(
        f"/api/data_unlabeled_table/?project={project.pk}"
    ).json()
...
def test_admin_table(
    ...
    response = client.get(f"/api/data_admin_table/{project.pk}/").json()
    assert (
        "detail" in response
        and "Invalid permission. Must be an admin" in response["detail"]
    )

    response = admin_client.get(f"/api/data_admin_table/{project.pk}/").json()
    ...
    response = admin_client.post(
        f"/api/label_admin_label/{data[1].pk}/", {"labelID": test_labels[0].pk}
    )
    response = admin_client.get(f"/api/data_admin_table/{project.pk}/").json()
    assert len(response["data"]) == 0
...
def test_multiple_admin_on_admin_annotation(
    ...
    response = client.get(f"/api/enter_coding_page/{project.pk}/").json()
    response2 = admin_client.get(f"/api/enter_coding_page/{project.pk}/").json()
    ...
    response = client.get(f"/api/check_admin_in_progress/{project.pk}/").json()
    response2 = admin_client.get(f"/api/check_admin_in_progress/{project.pk}/").json()
    ...
    response = client.get(f"/api/leave_coding_page/{project.pk}/").json()

    response2 = admin_client.get(f"/api/leave_coding_page/{project.pk}/").json()
    response2 = admin_client.get(f"/api/enter_coding_page/{project.pk}/").json()
...
def test_discard_data(
    ...
    response = client.post(f"/api/discard_data/{admin_data[0].data.pk}/").json()
    assert (
        "detail" in response
        and "Invalid permission. Must be an admin" in response["detail"]
    )
...
def test_restore_data(
    ...
    # check for admin privalidges
    response = client.post(f"/api/restore_data/{admin_data[0].data.pk}/").json()
    assert (
        "detail" in response
        and "Invalid permission. Must be an admin" in response["detail"]
    )
...
def test_recycle_bin_table(
    ...
    # check for admin privalidges
    response = client.get(f"/api/recycle_bin_table/{project.pk}/").json()
    assert (
        "detail" in response
        and "Invalid permission. Must be an admin" in response["detail"]
    )
    ...
    response = admin_client.get(f"/api/recycle_bin_table/{project.pk}/").json()
...
def test_admin_counts(
    ...
    for i in range(2):
        ...
        response = client.get(f"/api/data_admin_counts/{projects[i].pk}/").json()
        assert (
            "detail" in response
            and "Invalid permission. Must be an admin" in response["detail"]
        )
    ...
    response = admin_client.get(f"/api/data_admin_counts/{projects[0].pk}/").json()
    assert "detail" not in response and len(response["data"]) == 2
    assert list(response["data"].values()) == [0, 0]

    response = admin_client.get(f"/api/data_admin_counts/{projects[1].pk}/").json()
